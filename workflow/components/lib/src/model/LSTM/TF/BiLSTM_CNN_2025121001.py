...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "BiLSTM+CNN")
...
class LSTMModel:
    ...
    def evaluate_model(self):
        ...
        with open(os.path.join(OUTPUT_DIR, 'result_bilstm_cnn.txt'), 'w') as f:
            # 第一行：预测结果序列（空格分隔）
    ...
    def save_model(self, model_path='lstm_model.h5'):
        ...
        model_path = os.path.join(OUTPUT_DIR, f'bilstm_4_cnn_{int(time.time())}.h5')
        # 检测模型是否存在
        if self.model is None:
            print("模型尚未构建，无法保存")
            return
