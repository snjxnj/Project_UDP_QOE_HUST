"""
LSTM-CNN模型用于时间序列分类任务
在这个模型当中，我们结合了LSTM和卷积神经网络（CNN）的优势，以更好地捕捉时间序列数据中的时序特征和局部特征。
模型结构：
1. 输入层：接受时间序列数据作为输入。
2. LSTM层（2层）：用于捕捉时间序列数据中的时序特征。
...
class LSTMModel:
    ...
    def build_model(self):
        ...
        print(f"\n[模型参数量] Total Parameters: {total_params:,}")
        ...
        print(f"[模型计算量] FLOPs (Inference, batch=1): {flops.total_float_ops:,}")
