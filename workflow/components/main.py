...
if __name__ == "__main__":
    ...
    file_path = sys.argv[-1] if len(sys.argv) > 1 else "./targetList.txt"
    print(f"### Main Info: Driver file path is set to '{file_path}'.")
    print("---------------------------------")
    
    # 2. 调用read_Driver_File函数读取驱动文件
    print("### Main Info: Calling read_Driver_File function...")
    original_dataFrame = driverfile_processing.read_Driver_File(file_path)
    # 检查读取结果
    ...
    extractor_manager = extractor_container(legal_dataFrame_st, commands)
