"""
CNN-BiLSTM模型用于时间序列分类任务
在这个模型当中，我们结合了卷积神经网络（CNN）和双向LSTM的优势，以更好地捕捉时间序列数据中的局部特征和时序特征。
模型结构：
1. 输入层：接受时间序列数据作为输入。
2. 卷积层：用于捕捉时间序列数据中的局部特征。
3. 池化层：用于降低特征维度，并提取局部特征。
4. 双向LSTM层：用于捕捉时间序列数据中的时序特征。
5. 全连接层：用于将特征映射到分类结果。
6. 输出层：输出分类结果。
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "CNN+BiLSTM")
...
def setup_logging():
    ...
    logging.basicConfig(
        # 在文件名上添加实时时间戳，避免覆盖
        filename= 'logs/cnn_4_bilstm_model_{}_log.txt'.format(time.strftime('%Y%m%d_%H%M%S')),
...
class LSTMModel:
    ...
    def build_model(self):
        ...
        # 3. 添加双向LSTM层来进一步提取时序特征
        self.model.add(Bidirectional(LSTM(64, return_sequences=True)))
        self.model.add(Dropout(0.2))

        # 第3个双向LSTM层（可选）
        self.model.add(Bidirectional(LSTM(32, return_sequences=False)))  # 最后一层不需要return_sequences
        self.model.add(Dropout(0.2))

        # 第4个双向LSTM层（可选）
        # self.model.add(Bidirectional(LSTM(16, return_sequences=False)))  # 最后一层不需要return_sequences
        # self.model.add(Dropout(0.2))
        ...
        # 5. 输出层（二分类问题使用sigmoid，多分类使用softmax）
    ...
    def evaluate_model(self):
        ...
        # 保存预测结果和真实标签到文件
        print("\n正在保存预测结果和真实标签到result_cnn_bilstm.txt...")
        with open(os.path.join(OUTPUT_DIR, 'result_cnn_bilstm.txt'), 'w') as f:
            # 第一行：预测结果序列（空格分隔）
            f.write(' '.join(map(str, y_pred_classes.ravel())) + '\n')
            # 第二行：测试集label标签原始结果（空格分隔）
            f.write(' '.join(map(str, self.y_test.ravel())) + '\n')
        print("预测结果和真实标签已保存到result_cnn_bilstm.txt")
        logging.info("预测结果和真实标签已保存到result_cnn_bilstm.txt")
    ...
    def save_model(self, model_path='lstm_model.h5'):
        ...
        model_path = os.path.join(OUTPUT_DIR, f'cnn_4_bilstm_{int(time.time())}.h5')
        # 检测模型是否存在
        if self.model is None:
            print("模型尚未构建，无法保存")
            return
