...
from visual_06.visualier_in_DataPre import visualier_in_DataPre

if __name__ == "__main__":
    # 0. 讲程序打印的方向调整到指定log文件中
    ...
    legal_dataFrame_st = storage_processing.generate_storage(legal_dataFrame, output_dir="../output/data_preProcess")
    # 检查存储库生成结果
    ...
    # 14. 根据用户指令执行可视化操作
    if "-v" in commands:
        print("### Main Info: Visualizing the results...")
        visualizer = visualier_in_DataPre(legal_dataFrame_st, commands)
        visualizer.export_visual()
        print("### Main Info: Completed the visualization process.")
        print("-"*30 + "\n")
