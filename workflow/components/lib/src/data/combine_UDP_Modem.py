...
def process_modem_time(modem_time_str, udp_datetime):
    """
    处理Modem时间（新版本）：
    1. modem_time_str 现在是带日期的完整时间：YYYY-MM-DD HH:MM:SS.fff
    2. 直接解析为 datetime
    3. 四舍五入到秒级
    4. 不再使用 UDP 的日期，完全以 Modem 自己的日期为准
    """
    # 按完整格式解析，比如 "2025-10-17 20:59:30.598"
    modem_dt = pd.to_datetime(modem_time_str, format='%Y-%m-%d %H:%M:%S.%f')

    # 四舍五入到秒（0.5 秒进 1 秒），自动处理进位和跨日
    modem_dt_rounded = modem_dt.round('s')

    return modem_dt_rounded
...
def match_and_combine_files(udp_base_dir, modem_base_dir, output_dir):
    ...
    for scenario in ['gaming', 'meeting', 'video']:
        ...
        scenario_modem_files = glob.glob(os.path.join(modem_dir, f"{scenario}_*_mean.csv"))
...
def main():
    # 设置基础目录
    UDP_BASE_DIR = r"D:\XFC_files\code\UDP2026\data\processed\pcap_info"
    MODEM_BASE_DIR = r"D:\XFC_files\code\UDP2026\data\processed\modem_info\caculated_modem_data"
    OUTPUT_DIR = r"D:\XFC_files\code\UDP2026\data\processed\combine_info"
